    ...
    public class Plc : IDisposable
    {
        ...
        public object Read(string variable)
        {
            ...
            try
            {
                ...
                uint num = VarTypeCaller.ComputeStringHash(text2);
                int startByteAdr;
                int num3;
                if (num <= 752165258U)
                {
                ...
                num = VarTypeCaller.ComputeStringHash(text2);
                DataType dataType;
                if (num <= 3356228888U)
                {
                    if (num <= 3289118412U)
        ...
        {
            int structSize = VarTypeCaller.GetStructSize(structType);
            byte[] bytes = this.ReadBytes(DataType.DataBlock, db, startByteAdr, structSize);
            return VarTypeCaller.StructFromBytes(structType, bytes);
        }
        /// <summary>
        /// 读类
        /// </summary>
        ...
        public void ReadClass(object sourceClass, int db, int startByteAdr = 0)
        {
            ...
            int classSize = VarTypeCaller.GetClassSize(type);
            byte[] bytes = this.ReadBytes(DataType.DataBlock, db, startByteAdr, classSize);
            VarTypeCaller.ClassFromBytes(sourceClass, type, bytes);
        }
        /// <summary>
        /// 写字节
        /// </summary>
        ...
        public ErrorCode WriteBytes(DataType dataType, int db, int startByteAdr, byte[] value)
        {
            ...
            try
            {
                ...
                byteArray.Add(VarTypeCaller.WordToByteArray((ushort)(num - 1)));
                byteArray.Add(new byte[]
                {
                    0,
                    14
                });
                byteArray.Add(VarTypeCaller.WordToByteArray((ushort)(num + 4)));
                byteArray.Add(new byte[]
                {
                    5,
                    1,
                ...
                byteArray.Add(VarTypeCaller.WordToByteArray((ushort)num));
                byteArray.Add(VarTypeCaller.WordToByteArray((ushort)db));
                byteArray.Add((byte)dataType);
                int num3 = (int)((long)(startByteAdr * 8) / 65535L);
                byteArray.Add((byte)num3);
                byteArray.Add(VarTypeCaller.WordToByteArray((ushort)(startByteAdr * 8)));
                byteArray.Add(new byte[]
                {
                    0,
                    4
                });
                byteArray.Add(VarTypeCaller.WordToByteArray((ushort)(num * 8)));
                byteArray.Add(value);
                this._mSocket.Send(byteArray.Array, byteArray.Array.Length, SocketFlags.None);
                this._mSocket.Receive(array, 512, SocketFlags.None);
                if (array[21] != 255)
        ...
        public ErrorCode Write(DataType dataType, int db, int startByteAdr, object value)
        {
            ...
            uint num = VarTypeCaller.ComputeStringHash(name);
            byte[] value2;
            if (num <= 2341828857U)
            {
                if (num <= 1323747186U)
                {
                    if (num != 765439473U)
                    {
                        if (num != 1189326818U)
                        {
                            if (num == 1323747186U)
                            {
                                if (name == "UInt16")
                                {
                                    value2 = VarTypeCaller.WordToByteArray((ushort)value);
                                    goto IL_2B3;
                                }
                            }
                        }
                    ...
                    {
                        value2 = VarTypeCaller.IntToByteArray((short)value);
                        goto IL_2B3;
                    }
                }
                else if (num != 1615808600U)
                {
                    ...
                    {
                        value2 = VarTypeCaller.DWordToByteArray((uint[])value);
                        goto IL_2B3;
                    }
                }
                else if (name == "String")
                {
                    value2 = VarTypeCaller.StringToByteArray(value as string);
                    goto IL_2B3;
                }
            }
            else if (num <= 2711245919U)
            {
                if (num != 2386971688U)
                {
                    if (num != 2642490659U)
                    {
                        if (num == 2711245919U)
                        {
                            if (name == "Int32")
                            {
                                value2 = VarTypeCaller.DIntToByteArray((int)value);
                                goto IL_2B3;
                            }
                        }
                    }
                ...
                {
                    value2 = VarTypeCaller.DoubleToByteArray((double)value);
                    goto IL_2B3;
                }
            }
            else if (num <= 3509231420U)
            {
                ...
                {
                    value2 = VarTypeCaller.ByteToByteArray((byte)value);
                    goto IL_2B3;
                }
            }
        ...
        {
            List<byte> bytes = VarTypeCaller.StructToBytes(structValue).ToList<byte>();
        ...
        {
            List<byte> bytes = VarTypeCaller.ClassToBytes(classValue).ToList<byte>();
            return this.WriteMultipleBytes(bytes, db, startByteAdr);
        }
        /// <summary>
        /// 清除最后一次错误
        ...
        internal ErrorCode WriteMultipleBytes(List<byte> bytes, int db, int startByteAdr = 0)
        {
            ...
            {
                this.LastErrorCode = ErrorCode.WriteData;
                this.LastErrorString = "写数据是发生错误：" + ex.Message;
            }
            return errorCode;
        }
        /// <summary>
        ...
        internal ByteArray ReadHeaderPackage(int amount = 1)
        {
            ...
            byteArray.Add(VarTypeCaller.WordToByteArray((ushort)(2 + amount * 12)));
        ...
        internal ByteArray CreateReadDataRequestPackage(DataType dataType, int db, int startByteAdr, int count = 1)
        {
            ...
            byteArray.Add(VarTypeCaller.WordToByteArray((ushort)count));
            byteArray.Add(VarTypeCaller.WordToByteArray((ushort)db));
            byteArray.Add((byte)dataType);
            ...
            if (dataType == DataType.Counter || dataType == DataType.Timer)
            {
                byteArray.Add(VarTypeCaller.WordToByteArray((ushort)startByteAdr));
            }
            else
            {
                byteArray.Add(VarTypeCaller.WordToByteArray((ushort)(startByteAdr * 8)));
        ...
        static object ParseBytes(VarType varType, byte[] bytes, int varCount)
        {
            ...
            switch (varType)
            {
                ...
                case VarType.Word: return varCount == 1 ? VarTypeCaller.WordFromByteArray(bytes) : VarTypeCaller.WordToArray(bytes);
                case VarType.DWord: return varCount == 1 ? VarTypeCaller.DWordFromByteArray(bytes) : VarTypeCaller.DWordToArray(bytes);
                case VarType.Int: return varCount == 1 ? VarTypeCaller.IntFromByteArray(bytes) : VarTypeCaller.IntToArray(bytes);
                case VarType.DInt: return varCount == 1 ? VarTypeCaller.DIntFromByteArray(bytes) : VarTypeCaller.DIntToArray(bytes);
                case VarType.Real: return varCount == 1 ? VarTypeCaller.DoubleFromByteArray(bytes) : VarTypeCaller.DoubleToArray(bytes);
                case VarType.String: return VarTypeCaller.StringFromByteArray(bytes);
                case VarType.Timer: return varCount == 1 ? VarTypeCaller.TimerFromByteArray(bytes) : VarTypeCaller.TimerToArray(bytes);
                case VarType.Counter: return varCount == 1 ? VarTypeCaller.CounterFromByteArray(bytes) : VarTypeCaller.CounterToArray(bytes);
                default: return null;
            }
        }
        /// <summary>
