...
namespace System.Data.HardwareInterfaces
{
    ...
    public class AssemblyClassLoader
    {
        ...
        public static T CreateProxy<T>()
        {
            ...
            //var aopType = BulidType<T>(classType, moduleBuilder);
        ...
        private static Type BulidType<T>(Type classType, ModuleBuilder moduleBuilder)
        {
            string className = classType.Name + "_Proxy";
            //定义类型
            var typeBuilder = moduleBuilder.DefineType(className, TypeAttributes.Public | TypeAttributes.Sealed | TypeAttributes.Class, classType);//定义一个新类，继承classType
            var inspectorFieldBuilder = typeBuilder.DefineField("_inspector", typeof(T), FieldAttributes.Private | FieldAttributes.InitOnly);
            //构造函数 
            BuildCtor<T>(classType, inspectorFieldBuilder, typeBuilder);
            //构造方法
            BuildMethod<T>(classType, inspectorFieldBuilder, typeBuilder);
            Type aopType = typeBuilder.CreateType();
            return aopType;
        }

        private static void BuildMethod<T>(Type classType, FieldBuilder inspectorFieldBuilder, TypeBuilder typeBuilder)
        {
            ...
            foreach (var methodInfo in methodInfos)// 获取 虚函数 或 抽象函数
            {
                ...
                il.Emit(OpCodes.Callvirt, typeof(T).GetMethod("BeforeCall"));//调用BeforeCall
                ...
                il.Emit(OpCodes.Callvirt, typeof(T).GetMethod("AfterCall"));
                #endregion
        ...
        private static void BuildCtor<T>(Type classType, FieldBuilder inspectorFieldBuilder, TypeBuilder typeBuilder)
        {
            {  //定义 代理类的 构造器
                ...
                il.Emit(OpCodes.Call, typeof(T).GetMethod("Create", new[] { typeof(Type) }));
                //将结果保存到字段_inspector
