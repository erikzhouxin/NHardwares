...
namespace DeYaIceIpcSDK.Builder
{
    internal class Program
    {
        ...
        static string SUMMARY { get; } = "德亚车牌识别项目集成";
        static string PACKAGE_TAGS { get; } = "德亚;德亚SDK;IceIpcSDK;";
        public static String Config = "Debug";
        static string ProjName => "DeYaIceIpcSDK";
        static String DllNative => "\\native";
        //static String DllNative => "\\lib";
        static string DllVirtual => "\\plugins\\iceipcsdk";
        ...
        private static void write_nuspec_file_entry(string src, string target, XmlWriter f)
        {
            ...
            //f.WriteAttributeString("buildAction", "None");
            //f.WriteAttributeString("copyToOutput", "true");
        ...
        private static void GenNuspecLib(string dir_src)
        {
            ...
            }))
            {
                ...
                {
                    f.WriteStartElement("file");
                    f.WriteAttributeString("src", $"{DllVirtual.TrimStart('\\')}\\_._");
                    f.WriteAttributeString("target", $"lib/{sdk}{DllVirtual.Replace('\\', '/')}/_._");
                    f.WriteEndElement(); // file
                }
                foreach (var cpuTag in cpuTags)
                {
                    ...
                    //foreach (var item in Directory.GetFiles(Path.Combine(Src, "beans", platform)))
                    {
                        ...
                        write_nuspec_file_entry(Path.Combine("$cb_bin_path$", platform, fileName), $"runtimes\\{cpuTag}{DllNative}{DllVirtual}\\", f);
                    }
                }
                ...
                }))
                {
                    ...
                    foreach (var cpuTag in cpuTags)
                    {
                        ...
                        //foreach (var item in Directory.GetFiles(Path.Combine(Src, "beans", platform)))
                        {
                            ...
                            ftar.WriteAttributeString("Include", $"$(MSBuildThisFileDirectory)..\\..\\runtimes\\{cpuTag}{DllNative}{DllVirtual}\\");
                            ftar.WriteElementString("Link", $".{DllVirtual}\\");
                            ftar.WriteElementString("CopyToOutputDirectory", "PreserveNewest");
                            ftar.WriteElementString("Pack", "true");
                            ftar.WriteElementString("PackagePath", $"runtimes\\{cpuTag}{DllNative}{DllVirtual}\\");

                            ftar.WriteEndElement(); // Content
                            ftar.WriteStartElement("Content");
                            ftar.WriteAttributeString("Include", $"$(MSBuildThisFileDirectory)..\\..\\runtimes\\{cpuTag}{DllNative}{DllVirtual}\\{fileName}");
                            ftar.WriteElementString("Link", $".{DllVirtual}\\%(Filename)%(Extension)");
                            ftar.WriteElementString("CopyToOutputDirectory", "PreserveNewest");
                            ftar.WriteElementString("Pack", "true");
                            ftar.WriteElementString("PackagePath", $"runtimes\\{cpuTag}{DllNative}{DllVirtual}\\%(Filename)%(Extension)");
                            ftar.WriteEndElement(); // Content
                        }
                    }
