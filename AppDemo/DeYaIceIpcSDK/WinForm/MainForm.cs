...
namespace TestDll
{
    public partial class MainForm : Form
    {
        ...
        public delegate void IceIpcUpdatePlateInfo(string strIP, string strNum, string strColor, uint nVehicleColor,
            uint nAlarmType, short nVehiclType, uint nCapTime, int index, string strLogName, ICE_RECT_S stVlprInfo);
        public delegate void IceIpcUpdateStatus(int index, int type);
        public delegate void IceIpcUpdateTriggerStatus(int index, uint nStatus);
        public delegate void IceIpcUpdatePortInfo(string strIp, uint len, int index, string data, int type);
        public delegate void IceIpcUpdateDeviceEvent(string strIP, uint nType, uint ndata1, uint ndata2, uint ndata3, uint ndata4, int nFlag, int road);

        private IceIpcUpdatePlateInfo updatePlateInfo;
        private IceIpcUpdateStatus updateStatus;
        private IceIpcUpdateTriggerStatus triggerStatus;
        private IceIpcUpdatePortInfo updatePortInfo;
        private IceIpcUpdateDeviceEvent updateDeviceEvent;

        private ICE_IPCSDK_OnPlate onPlate;
        private ICE_IPCSDK_OnFrame_Planar onFrame;
        private ICE_IPCSDK_OnPastPlate onPastPlate;
        private ICE_IPCSDK_OnSerialPort onSerialPortRS485;
        ...
        {
            //存图
            var saveRes = SaveStorePic(picData);
            if (!saveRes.IsSuccess) { return; }
            //保存特征码
            if (bIsPlate || null == fResFuture) { return; }
            string strFileName = Path.Combine(Path.GetFullPath("Temp"), "vbr_record.txt");
            ...
            try
            {
                File.AppendAllText(strFileName, saveRes.Data + " " + strNumber + " " + strContent + "\r\n");
            }
            catch { }
        }
        /// <summary>
                ...
                try
                {
                    ...
                    using (var fs = new MemoryStream())
                    {
                        using (BinaryWriter bw = new BinaryWriter(fs))
                        {
                            ...
                            bw.Flush();
                            var picData = fs.ToArray();
                            var saveRes = SaveStorePic(picData, ".bmp");
                        }
                    }
                }
        ...
        public void SDK_OnSerialPortRS485(System.IntPtr pvParam,
            [System.Runtime.InteropServices.InAttribute()][System.Runtime.InteropServices.MarshalAsAttribute(System.Runtime.InteropServices.UnmanagedType.LPStr)] string pcIP,
        ...
        private void buttonConnect_Click(object sender, EventArgs e)
        {
            if (radioButton_Preview.Checked) //选择连接方式为：视频预览
            {
                ...
                if (videoHwnd != IntPtr.Zero)
                {
                    ...
                    {
                        pUid[0] = _proxy.ICE_IPCSDK_OpenPreview_Passwd(textBoxIP1.Text, textBoxPassword.Text, 1, 1,
                    ...
                    {
                        //使用不带密码的接口连接相机
                        pUid[0] = _proxy.ICE_IPCSDK_OpenPreview(textBoxIP1.Text, 1, 1, (uint)videoHwnd, onPlate, new IntPtr(0));
                        if (pUid[0] == IntPtr.Zero)
                        {
            ...
            else if (radioButton_Frame.Checked == true) //选择连接方法为：图像解码
            {
                ...
                {
                    //调用带密码的接口连接相机
                    pUid[0] = _proxy.ICE_IPCSDK_Open_Passwd(textBoxIP1.Text, textBoxPassword.Text, 1, 554, 8117, 8080, 1, 0, IntPtr.Zero, 0, IntPtr.Zero);
                    if (pUid[0] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机1连接失败,密码错误或者网络不好！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                {
                    //调用不带密码的接口连接相机
                    pUid[0] = _proxy.ICE_IPCSDK_Open(textBoxIP1.Text, 1, 554, 8117, 8080, 1, 0, IntPtr.Zero, 0, IntPtr.Zero);
                    if (pUid[0] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机1连接失败！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                _proxy.ICE_IPCSDK_SetFrameCallback(pUid[0], onFrame, new IntPtr(0));//设置获得解码出的一帧图像的相关回调函数
                _proxy.ICE_IPCSDK_SetPlateCallback(pUid[0], onPlate, new IntPtr(0));//设置获取车牌识别数据的回调函数
            }
            else if (radioButtonOnlyConn.Checked == true) //选择连接方式为：连接与码流分开
            {
                ...
                {
                    //调用带密码的连接接口
                    pUid[0] = _proxy.ICE_IPCSDK_OpenDevice_Passwd(textBoxIP1.Text, textBoxPassword.Text);
                    if (pUid[0] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机1连接失败,密码错误或者网络不好！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                {
                    //调用不带密码的连接接口
                    pUid[0] = _proxy.ICE_IPCSDK_OpenDevice(textBoxIP1.Text);
                    if (pUid[0] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机1连接失败！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                if (videoHwnd != IntPtr.Zero)
                {
                    ...
                    UInt32 ret = _proxy.ICE_IPCSDK_StartStream(pUid[0], 1, (UInt32)videoHwnd);
                    if (ret == 0)
                    {
                        _proxy.ICE_IPCSDK_StopStream(pUid[0]);//连接视频失败，调用断开视频接口释放资源
                        return;
                    }
                }
                else
                ...
                _proxy.ICE_IPCSDK_SetPlateCallback(pUid[0], onPlate, new IntPtr(0));//设置车牌识别数据回调
                buttonStartVideo1.Enabled = true;
                buttonStartVideo1.Text = "结束视频";
            }

            //设置断网续传回调
            _proxy.ICE_IPCSDK_SetPastPlateCallBack(pUid[0], onPastPlate, new IntPtr(0));

            //设置RS485透明串口回调
            _proxy.ICE_IPCSDK_SetSerialPortCallBack(pUid[0], onSerialPortRS485, new IntPtr(0));//485
            //设置RS232透明串口回调
            _proxy.ICE_IPCSDK_SetSerialPortCallBack_RS232(pUid[0], onSerialPortRS232, new IntPtr(0));//232

            //IO变化事件
            _proxy.ICE_IPCSDK_SetIOEventCallBack(pUid[0], onIOEvent, new IntPtr(0));
        ...
        {
            _proxy.ICE_IPCSDK_Init(); //调用全局初始化
            ...
            onSerialPortRS485 = new ICE_IPCSDK_OnSerialPort(SDK_OnSerialPortRS485);
            ...
            updatePlateInfo = new IceIpcUpdatePlateInfo(ShowCount);
            updateStatus = new IceIpcUpdateStatus(ShowStatus);
            triggerStatus = new IceIpcUpdateTriggerStatus(ShowTriggerStatus);
            updatePortInfo = new IceIpcUpdatePortInfo(ShowPortData);
            updateDeviceEvent = new IceIpcUpdateDeviceEvent(ShowDeviceEvent);
            ...
            _proxy.ICE_IPCSDK_LogConfig(m_bEnableLog, m_strLogPath);//日志配置

            _proxy.ICE_IPCSDK_SetDeviceEventCallBack(IntPtr.Zero, onDeviceEvent, new IntPtr(0));
            ...
            uint nPosJpeg, string nColorJpeg, uint nDateTimeJpeg, uint nAlgo, uint nCustomJpeg, string bstrCustomJpeg)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                if (pUid[i] != IntPtr.Zero)
                {
                    ...
                    _proxy.ICE_IPCSDK_SetOSDCfg(pUid[i], ref osdInfo);//调用设置OSD参数接口
                }
            }
        }
            ...
            for (int i = 0; i < 4; i++)
            {
                if (pUid[i] != IntPtr.Zero)
                {
                    if (bRecord[i])//在进行录像，则关闭录像
                    {
                        ...
                        bRecord[i] = false;
                        _proxy.ICE_IPCSDK_StopRecord(pUid[i]);//调用关闭录像接口
                    }
                    _proxy.ICE_IPCSDK_Close(pUid[i]);
                    pUid[i] = IntPtr.Zero;
                }
            }
        ...
        private void buttonDisconnect_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (bTalk[0]) //在进行对讲
                {
                    _proxy.ICE_IPCSDK_EndTalk(pUid[0]);//结束对讲
                    button_talk.Text = "对讲";
                    bTalk[0] = false;
                }
                ...
                if (bRecord[0]) //在进行录像
                {
                    ...
                    _proxy.ICE_IPCSDK_StopRecord(pUid[0]); //结束录像
                }

                mutexThread.WaitOne();
                _proxy.ICE_IPCSDK_Close(pUid[0]);//断开连接
                pUid[0] = IntPtr.Zero;
                mutexThread.ReleaseMutex();
            }
            catch (System.Exception ex)
        ...
        private void buttonConnect2_Click(object sender, EventArgs e)
        {
            if (radioButton_Preview.Checked == true)//选择连接方式为：视频预览
            {
                ...
                if (videoHwnd != IntPtr.Zero)
                {
                    if (checkBoxPassword.Checked == true)//使用密码连接
                    {
                        pUid[1] = _proxy.ICE_IPCSDK_OpenPreview_Passwd(textBoxIP2.Text, textBoxPassword.Text, 1, 1,
                    ...
                    {
                        //使用不带密码的接口连接相机
                        pUid[1] = _proxy.ICE_IPCSDK_OpenPreview(textBoxIP2.Text, 1, 1, (uint)videoHwnd, onPlate, new IntPtr(1));
                        if (pUid[1] == IntPtr.Zero)
                        {
            ...
            else if (radioButton_Frame.Checked == true)//选择连接方法为：图像解码
            {
                ...
                {
                    //调用带密码的接口连接相机
                    pUid[1] = _proxy.ICE_IPCSDK_Open_Passwd(textBoxIP2.Text, textBoxPassword.Text, 1, 554, 8117, 8080, 1, 0, IntPtr.Zero, 0, IntPtr.Zero);
                    if (pUid[1] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机2连接失败，密码错误或者网络不好！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                {
                    //调用不带密码的接口连接相机
                    pUid[1] = _proxy.ICE_IPCSDK_Open(textBoxIP2.Text, 1, 554, 8117, 8080, 1, 0, IntPtr.Zero, 0, IntPtr.Zero);
                    if (pUid[1] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机2连接失败！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                //设置获得解码出的一帧图像的相关回调函数
                _proxy.ICE_IPCSDK_SetFrameCallback(pUid[1], onFrame, new IntPtr(1));
                //设置获取车牌识别数据的回调函数
                _proxy.ICE_IPCSDK_SetPlateCallback(pUid[1], onPlate, new IntPtr(1));
            }
            else if (radioButtonOnlyConn.Checked == true)//选择连接方式为：连接与码流分开
            {
                if (checkBoxPassword.Checked == true)//使用密码连接
                {
                    //调用带密码的连接接口
                    pUid[1] = _proxy.ICE_IPCSDK_OpenDevice_Passwd(textBoxIP2.Text, textBoxPassword.Text);
                    if (pUid[1] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机2连接失败，密码错误或者网络不好！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                {
                    //调用不带密码的连接接口
                    pUid[1] = _proxy.ICE_IPCSDK_OpenDevice(textBoxIP2.Text);
                    if (pUid[1] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机2连接失败！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                {
                    //连接视频，因为ICE_IPCSDK_OpenDevice不带视频流；如果不需要视频，可不调用该接口
                    UInt32 ret = _proxy.ICE_IPCSDK_StartStream(pUid[1], 1, (UInt32)videoHwnd);
                    if (ret == 0)
                    {
                        _proxy.ICE_IPCSDK_StopStream(pUid[1]);//连接视频失败，调用断开视频接口释放资源
                        return;
                    }
                }
                else
            ...
            //设置断网续传回调
            _proxy.ICE_IPCSDK_SetPastPlateCallBack(pUid[1], onPastPlate, new IntPtr(1));

            //设置RS485透明串口回调
            _proxy.ICE_IPCSDK_SetSerialPortCallBack(pUid[1], onSerialPortRS485, new IntPtr(1));//485
            //设置RS232透明串口回调
            _proxy.ICE_IPCSDK_SetSerialPortCallBack_RS232(pUid[1], onSerialPortRS232, new IntPtr(1));//232

            //IO状态变化事件
            _proxy.ICE_IPCSDK_SetIOEventCallBack(pUid[1], onIOEvent, new IntPtr(1));
        ...
        private void buttonDisconnect2_Click(object sender, EventArgs e)
        {
            ...
            if (bTalk[1])//在进行对讲
            {
                _proxy.ICE_IPCSDK_EndTalk(pUid[1]);//结束对讲
            ...
            if (bRecord[1])//在进行录像
            {
                ...
                _proxy.ICE_IPCSDK_StopRecord(pUid[1]);//结束录像
            }

            mutexThread.WaitOne();
            _proxy.ICE_IPCSDK_Close(pUid[1]);//断开连接
            pUid[1] = IntPtr.Zero;
            mutexThread.ReleaseMutex();
        ...
        private void buttonConnect3_Click(object sender, EventArgs e)
        {
            if (radioButton_Preview.Checked == true)//选择连接方式为：视频预览
            {
                ...
                if (videoHwnd != IntPtr.Zero)
                {
                    if (checkBoxPassword.Checked == true)//使用密码连接
                    {
                        ...
                        pUid[2] = _proxy.ICE_IPCSDK_OpenPreview_Passwd(textBoxIP3.Text, textBoxPassword.Text, 1, 1, (uint)videoHwnd, onPlate, new IntPtr(2));
                    ...
                    {
                        //使用不带密码的接口连接相机
                        pUid[2] = _proxy.ICE_IPCSDK_OpenPreview(textBoxIP3.Text, 1, 1, (uint)videoHwnd, onPlate, new IntPtr(2));
                        if (pUid[2] == IntPtr.Zero)
                        {
            ...
            else if (radioButton_Frame.Checked == true)//选择连接方法为：图像解码
            {
                ...
                {
                    //调用带密码的接口连接相机
                    pUid[2] = _proxy.ICE_IPCSDK_Open_Passwd(textBoxIP3.Text, textBoxPassword.Text, 1, 554, 8117, 8080, 1, 0, IntPtr.Zero, 0, IntPtr.Zero);
                    if (pUid[2] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机3连接失败，密码错误或者网络不好！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                {
                    //调用不带密码的接口连接相机
                    pUid[2] = _proxy.ICE_IPCSDK_Open(textBoxIP3.Text, 1, 554, 8117, 8080, 1, 0, IntPtr.Zero, 0, IntPtr.Zero);
                    if (pUid[2] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机3连接失败！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                //设置获得解码出的一帧图像的相关回调函数
                _proxy.ICE_IPCSDK_SetFrameCallback(pUid[2], onFrame, new IntPtr(2));
                //设置获取车牌识别数据的回调函数
                _proxy.ICE_IPCSDK_SetPlateCallback(pUid[2], onPlate, new IntPtr(2));
            }
            else if (radioButtonOnlyConn.Checked == true)//选择连接方式为：连接与码流分开
            {
                if (checkBoxPassword.Checked == true)//使用密码连接
                {
                    //调用带密码的连接接口
                    pUid[2] = _proxy.ICE_IPCSDK_OpenDevice_Passwd(textBoxIP3.Text, textBoxPassword.Text);
                    if (pUid[2] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机3连接失败，密码错误或者网络不好！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                {
                    //调用不带密码的连接接口
                    pUid[2] = _proxy.ICE_IPCSDK_OpenDevice(textBoxIP3.Text);
                    if (pUid[2] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机3连接失败！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                {
                    //连接视频，因为ICE_IPCSDK_OpenDevice不带视频流；如果不需要视频，可不调用该接口
                    UInt32 ret = _proxy.ICE_IPCSDK_StartStream(pUid[2], 1, (UInt32)videoHwnd);
                    if (ret == 0)
                    {
                        _proxy.ICE_IPCSDK_StopStream(pUid[2]);//连接视频失败，调用断开视频接口释放资源
                        return;
                    }
                }
                else
            ...
            //设置断网续传回调
            _proxy.ICE_IPCSDK_SetPastPlateCallBack(pUid[2], onPastPlate, new IntPtr(2));

            //设置RS485透明串口回调
            _proxy.ICE_IPCSDK_SetSerialPortCallBack(pUid[2], onSerialPortRS485, new IntPtr(2));//485
            //设置RS232透明串口回调
            _proxy.ICE_IPCSDK_SetSerialPortCallBack_RS232(pUid[2], onSerialPortRS232, new IntPtr(2));//232

            //IO状态变化事件
            _proxy.ICE_IPCSDK_SetIOEventCallBack(pUid[2], onIOEvent, new IntPtr(2));
        ...
        private void buttonDisconnect3_Click(object sender, EventArgs e)
        {
            ...
            if (bTalk[2])//在进行对讲
            {
                _proxy.ICE_IPCSDK_EndTalk(pUid[2]);//结束对讲
            ...
            if (bRecord[2])//在进行录像
            {
                ...
                _proxy.ICE_IPCSDK_StopRecord(pUid[2]);//结束录像
            }

            mutexThread.WaitOne();
            _proxy.ICE_IPCSDK_Close(pUid[2]);//断开连接
            pUid[2] = IntPtr.Zero;
            mutexThread.ReleaseMutex();
        ...
        private void buttonConnect4_Click(object sender, EventArgs e)
        {
            if (radioButton_Preview.Checked == true)//选择连接方式为：视频预览
            {
                ...
                if (videoHwnd != IntPtr.Zero)
                {
                    ...
                    {
                        //使用带密码的接口连接相机，并且设置车牌识别数据回调onPlate
                        pUid[3] = _proxy.ICE_IPCSDK_OpenPreview_Passwd(textBoxIP4.Text, textBoxPassword.Text, 1, 1, (uint)videoHwnd, onPlate, new IntPtr(3));
                    ...
                    {
                        //使用不带密码的接口连接相机
                        pUid[3] = _proxy.ICE_IPCSDK_OpenPreview(textBoxIP4.Text, 1, 1, (uint)videoHwnd, onPlate, new IntPtr(3));
                        if (pUid[3] == IntPtr.Zero)
                        {
            ...
            else if (radioButton_Frame.Checked == true)//选择连接方法为：图像解码
            {
                ...
                {
                    //调用带密码的接口连接相机
                    pUid[3] = _proxy.ICE_IPCSDK_Open_Passwd(textBoxIP4.Text, textBoxPassword.Text, 1, 554, 8117, 8080, 1, 0, IntPtr.Zero, 0, IntPtr.Zero);
                    if (pUid[3] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机4连接失败，密码错误或者网络不好！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                {
                    //调用不带密码的接口连接相机
                    pUid[3] = _proxy.ICE_IPCSDK_Open(textBoxIP4.Text, 1, 554, 8117, 8080, 1, 0, IntPtr.Zero, 0, IntPtr.Zero);
                    if (pUid[3] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机4连接失败！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                //设置获得解码出的一帧图像的相关回调函数
                _proxy.ICE_IPCSDK_SetFrameCallback(pUid[3], onFrame, new IntPtr(3));
                //设置获取车牌识别数据的回调函数
                _proxy.ICE_IPCSDK_SetPlateCallback(pUid[3], onPlate, new IntPtr(3));
            }
            else if (radioButtonOnlyConn.Checked == true)//选择连接方式为：连接与码流分开
            {
                if (checkBoxPassword.Checked == true)//使用密码连接
                {
                    //调用带密码的连接接口
                    pUid[3] = _proxy.ICE_IPCSDK_OpenDevice_Passwd(textBoxIP4.Text, textBoxPassword.Text);
                    if (pUid[3] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机4连接失败，密码错误或者网络不好！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                {
                    //调用不带密码的连接接口
                    pUid[3] = _proxy.ICE_IPCSDK_OpenDevice(textBoxIP4.Text);
                    if (pUid[3] == IntPtr.Zero)
                    {
                        MessageBox.Show("相机4连接失败！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                ...
                {
                    //连接视频，因为ICE_IPCSDK_OpenDevice不带视频流；如果不需要视频，可不调用该接口
                    UInt32 ret = _proxy.ICE_IPCSDK_StartStream(pUid[3], 1, (UInt32)videoHwnd);
                    if (ret == 0)
                    {
                        _proxy.ICE_IPCSDK_StopStream(pUid[3]);//连接视频失败，调用断开视频接口释放资源
                        return;
                    }
                }
                else
                ...
                _proxy.ICE_IPCSDK_SetPlateCallback(pUid[3], onPlate, new IntPtr(3));//设置车牌识别数据回调
                buttonStartVideo4.Enabled = true;
                buttonStartVideo4.Text = "结束视频";
            }
            ...
            //设置断网续传回调
            _proxy.ICE_IPCSDK_SetPastPlateCallBack(pUid[3], onPastPlate, new IntPtr(3));

            //设置RS485透明串口回调
            _proxy.ICE_IPCSDK_SetSerialPortCallBack(pUid[3], onSerialPortRS485, new IntPtr(3));//485
            //设置RS232透明串口回调
            _proxy.ICE_IPCSDK_SetSerialPortCallBack_RS232(pUid[3], onSerialPortRS232, new IntPtr(3));//232

            //IO状态变化事件
            _proxy.ICE_IPCSDK_SetIOEventCallBack(pUid[3], onIOEvent, new IntPtr(3));
        ...
        private void buttonDisconnect4_Click(object sender, EventArgs e)
        {
            ...
            if (bTalk[3])//在进行对讲
            {
                _proxy.ICE_IPCSDK_EndTalk(pUid[3]);//结束对讲
            ...
            if (bRecord[3])//在进行录像
            {
                ...
                _proxy.ICE_IPCSDK_StopRecord(pUid[3]);//结束录像
            }

            mutexThread.WaitOne();
            _proxy.ICE_IPCSDK_Close(pUid[3]);//断开连接
            pUid[3] = IntPtr.Zero;
            mutexThread.ReleaseMutex();
        ...
        private void button_openGate_Click(object sender, EventArgs e)
        {
            if (pUid[0] != IntPtr.Zero)
            {
                _proxy.ICE_IPCSDK_OpenGate(pUid[0]);//开闸
            }
        }
        ...
        private void button_openGate2_Click(object sender, EventArgs e)
        {
            if (pUid[1] != IntPtr.Zero)
            {
                _proxy.ICE_IPCSDK_OpenGate(pUid[1]);//开闸
            }
        }
        ...
        private void button_openGate3_Click(object sender, EventArgs e)
        {
            if (pUid[2] != IntPtr.Zero)
            {
                _proxy.ICE_IPCSDK_OpenGate(pUid[2]);//开闸
            }
        }
        ...
        private void button_openGate4_Click(object sender, EventArgs e)
        {
            if (pUid[3] != IntPtr.Zero)
            {
                _proxy.ICE_IPCSDK_OpenGate(pUid[3]);//开闸
            }
        }
        ...
        private void button_trigger_Click(object sender, EventArgs e)
        {
            if (pUid[0] != IntPtr.Zero)
            {
                ...
                uint success = _proxy.ICE_IPCSDK_Trigger(pUid[0], strNum, strColor, pdata, 1048576, ref len);//软触发
                //if (1 == success && len > 0)
                //{
                //    if (len > 0)
                //    {
        ...
        private void button_trigger2_Click(object sender, EventArgs e)
        {
            if (pUid[1] != IntPtr.Zero)
            {
                ...
                uint len = 0;
                byte[] pdata = new byte[1048576];
                uint success = _proxy.ICE_IPCSDK_Trigger(pUid[1], strNum, strColor, pdata, 1048576, ref len);//软触发
                //if (1 == success)
                //{
                //    if (len > 0)
                //    {
        ...
        private void button_trigger3_Click(object sender, EventArgs e)
        {
            if (pUid[2] != IntPtr.Zero)
            {
                ...
                uint len = 0;
                byte[] pdata = new byte[1048576];
                uint success = _proxy.ICE_IPCSDK_Trigger(pUid[2], strNum, strColor, pdata, 1048576, ref len);//软触发
                //if (1 == success)
                //{
                //    if (len > 0)
                //    {
        ...
        private void button_trigger4_Click(object sender, EventArgs e)
        {
            if (pUid[3] != IntPtr.Zero)
            {
                ...
                uint len = 0;
                byte[] pdata = new byte[1048576];
                uint success = _proxy.ICE_IPCSDK_Trigger(pUid[3], strNum, strColor, pdata, 1048576, ref len);//软触发
                //if (1 == success)
                //{
                //    if (len > 0)
                //    {
        ...
        private void button_reboot_Click(object sender, EventArgs e)
        {
            if (pUid[0] != IntPtr.Zero)
            {
                _proxy.ICE_IPCSDK_Reboot(pUid[0]);//重启
            }
        }
        ...
        private void button_reboot2_Click(object sender, EventArgs e)
        {
            if (pUid[1] != IntPtr.Zero)
            {
                _proxy.ICE_IPCSDK_Reboot(pUid[1]);//重启
            }
        }
        ...
        private void button_reboot3_Click(object sender, EventArgs e)
        {
            if (pUid[2] != IntPtr.Zero)
            {
                _proxy.ICE_IPCSDK_Reboot(pUid[2]);//重启
            }
        }
        ...
        private void button_reboot4_Click(object sender, EventArgs e)
        {
            if (pUid[3] != IntPtr.Zero)
            {
                _proxy.ICE_IPCSDK_Reboot(pUid[3]);//重启
            }
        }
        ...
        private void button_talk_Click(object sender, EventArgs e)
        {
            if (pUid[0] != IntPtr.Zero)
            {
                if (!bTalk[0])//开始对讲
                {
                    uint success = _proxy.ICE_IPCSDK_BeginTalk(pUid[0]);
                    if (success == 0)
                    {
                ...
                {
                    _proxy.ICE_IPCSDK_EndTalk(pUid[0]);
                    button_talk.Text = "对讲";
                    bTalk[0] = false;
                }
            }
        ...
        private void button_talk2_Click(object sender, EventArgs e)
        {
            if (pUid[1] != IntPtr.Zero)
            {
                if (!bTalk[1])//开始对讲
                {
                    uint success = _proxy.ICE_IPCSDK_BeginTalk(pUid[1]);
                    if (success == 0)
                    {
                        MessageBox.Show("对讲启动失败！");
                        return;
                ...
                {
                    _proxy.ICE_IPCSDK_EndTalk(pUid[1]);
                    button_talk2.Text = "对讲";
                    bTalk[1] = false;
                }
            }
        ...
        private void button_talk3_Click(object sender, EventArgs e)
        {
            if (pUid[2] != IntPtr.Zero)
            {
                if (!bTalk[2])//开始对讲
                {
                    uint success = _proxy.ICE_IPCSDK_BeginTalk(pUid[2]);
                    if (success == 0)
                    {
                        MessageBox.Show("对讲启动失败！");
                        return;
                ...
                {
                    _proxy.ICE_IPCSDK_EndTalk(pUid[2]);
                    button_talk3.Text = "对讲";
                    bTalk[2] = false;
                }
            }
        ...
        private void button_talk4_Click(object sender, EventArgs e)
        {
            if (pUid[3] != IntPtr.Zero)
            {
                if (!bTalk[3])//开始对讲
                {
                    uint success = _proxy.ICE_IPCSDK_BeginTalk(pUid[3]);
                    if (success == 0)
                    {
                        MessageBox.Show("对讲启动失败！");
                        return;
                ...
                {
                    _proxy.ICE_IPCSDK_EndTalk(pUid[3]);
                    button_talk4.Text = "对讲";
                    bTalk[3] = false;
                }
            }
        ...
        private void buttonSyncTime1_Click(object sender, EventArgs e)
        {
            if (IntPtr.Zero == pUid[0])
                return;
            //时间同步，相机采用utc时间
            _proxy.ICE_IPCSDK_SyncTime(pUid[0], (UInt16)DateTime.UtcNow.Year, (byte)DateTime.UtcNow.Month,
        ...
        private void buttonSyncTime2_Click(object sender, EventArgs e)
        {
            if (pUid[1] == IntPtr.Zero)
                return;
            //时间同步，相机采用utc时间
            _proxy.ICE_IPCSDK_SyncTime(pUid[1], (UInt16)DateTime.UtcNow.Year, (byte)DateTime.UtcNow.Month,
                (byte)DateTime.UtcNow.Day, (byte)DateTime.UtcNow.Hour, (byte)DateTime.UtcNow.Minute, (byte)DateTime.UtcNow.Second);
        }
        ...
        private void buttonSyncTime3_Click(object sender, EventArgs e)
        {
            if (pUid[2] == IntPtr.Zero)
                return;
            //时间同步，相机采用utc时间
            _proxy.ICE_IPCSDK_SyncTime(pUid[2], (UInt16)DateTime.UtcNow.Year, (byte)DateTime.UtcNow.Month,
                (byte)DateTime.UtcNow.Day, (byte)DateTime.UtcNow.Hour, (byte)DateTime.UtcNow.Minute, (byte)DateTime.UtcNow.Second);
        }
        ...
        private void buttonSyncTime4_Click(object sender, EventArgs e)
        {
            if (pUid[3] == IntPtr.Zero)
                return;
            //时间同步，相机采用utc时间
            _proxy.ICE_IPCSDK_SyncTime(pUid[3], (UInt16)DateTime.UtcNow.Year, (byte)DateTime.UtcNow.Month,
                (byte)DateTime.UtcNow.Day, (byte)DateTime.UtcNow.Hour, (byte)DateTime.UtcNow.Minute, (byte)DateTime.UtcNow.Second);
        }
        ...
        private void buttonSPortSend1_Click(object sender, EventArgs e)
        {
            ...
            //发送RS485数据
            _proxy.ICE_IPCSDK_TransSerialPort(pUid[0], byteArray, (UInt32)byteArray.Length);
            ...
            _proxy.ICE_IPCSDK_TransSerialPort(pUid[0], byteArray_16, (UInt32)byteArray_16.Length);
        }
        ...
        private void buttonSPortSend2_Click(object sender, EventArgs e)
        {
            ...
            byte[] byteArray = System.Text.Encoding.Default.GetBytes(strSendData);
            //发送RS485数据
            _proxy.ICE_IPCSDK_TransSerialPort(pUid[1], byteArray, (UInt32)byteArray.Length);
            ...
            _proxy.ICE_IPCSDK_TransSerialPort(pUid[1], byteArray_16, (UInt32)byteArray_16.Length);
        }
        ...
        private void buttonSPortSend3_Click(object sender, EventArgs e)
        {
            ...
            byte[] byteArray = System.Text.Encoding.Default.GetBytes(strSendData);
            //发送RS485数据
            _proxy.ICE_IPCSDK_TransSerialPort(pUid[2], byteArray, (UInt32)byteArray.Length);
            ...
            _proxy.ICE_IPCSDK_TransSerialPort(pUid[2], byteArray_16, (UInt32)byteArray_16.Length);
        }
        ...
        private void buttonSPortSend4_Click(object sender, EventArgs e)
        {
            ...
            byte[] byteArray = System.Text.Encoding.Default.GetBytes(strSendData);
            //发送RS485数据
            _proxy.ICE_IPCSDK_TransSerialPort(pUid[3], byteArray, (UInt32)byteArray.Length);
            ...
            _proxy.ICE_IPCSDK_TransSerialPort(pUid[3], byteArray_16, (UInt32)byteArray_16.Length);
        }
        ...
        private void buttonCapture1_Click(object sender, EventArgs e)
        {
            if (pUid[0] != IntPtr.Zero)
            {
                uint pLen = 0;
                byte[] pdata = new byte[1048576];
                UInt32 success = _proxy.ICE_IPCSDK_Capture(pUid[0], pdata, 1048576, ref pLen);//获取一张抓拍图
                if (1 == success && pLen > 0)
                {
                    if (pLen > 0)
                    {
        ...
        private void buttonCapture2_Click(object sender, EventArgs e)
        {
            if (pUid[1] != IntPtr.Zero)
            {
                uint len = 0;
                byte[] pdata = new byte[1048576];
                UInt32 success = _proxy.ICE_IPCSDK_Capture(pUid[1], pdata, 1048576, ref len);//获取一张抓拍图
                if (1 == success && len > 0)
                {
                    if (len > 0)
                    {
        ...
        private void buttonCapture3_Click(object sender, EventArgs e)
        {
            if (pUid[2] != IntPtr.Zero)
            {
                uint len = 0;
                byte[] pdata = new byte[1048576];
                UInt32 success = _proxy.ICE_IPCSDK_Capture(pUid[2], pdata, 1048576, ref len);//获取一张抓拍图
                if (1 == success && len > 0)
                {
                    if (len > 0)
                    {
        ...
        private void buttonCapture4_Click(object sender, EventArgs e)
        {
            if (pUid[3] != IntPtr.Zero)
            {
                uint len = 0;
                byte[] pdata = new byte[1048576];
                UInt32 success = _proxy.ICE_IPCSDK_Capture(pUid[3], pdata, 1048576, ref len);//获取一张抓拍图
                if (1 == success && len > 0)
                {
                    if (len > 0)
                    {
        ...
        private void GetStatus()
        {
            while (true)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    {
                        //mutexThread.WaitOne();
                        nCurrentStatus[i] = _proxy.ICE_IPCSDK_GetStatus(pUid[i]);//获取连接状态
                        //mutexThread.ReleaseMutex();
                        if (nCurrentStatus[i] != nStatus[i] && pUid[i] != IntPtr.Zero)
                        {
                            //mutexThread.WaitOne();
                            _proxy.ICE_IPCSDK_GetDevID(pUid[i], strMac[i]);//获取相机mac地址
                            //mutexThread.ReleaseMutex();
        ...
        private void openGate()
        {
            while (true)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    //mutexThread.WaitOne();
                    UInt32 success = _proxy.ICE_IPCSDK_OpenGate(pUid[i]);//打开道闸
                    //mutexThread.ReleaseMutex();
                    if (success == 1)
                    {
                        nGateNum[i]++;
        ...
        private void trigger()
        {
            while (true)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    //mutexThread.WaitOne();
                    uint success = _proxy.ICE_IPCSDK_TriggerExt(pUid[i]);
                    //mutexThread.ReleaseMutex();
                    if (success == 1)
                    {
                        nTriggerNum[i]++;
        ...
        private void openGate2()
        {
            while (true)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    //mutexThread.WaitOne();
                    UInt32 success = _proxy.ICE_IPCSDK_ControlAlarmOut(pUid[i], 1);//打开道闸2
                    //mutexThread.ReleaseMutex();
                    if (success == 1)
                    {
                        nGateNum[i]++;
        ...
        private void sendRS485()
        {
            while (true)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    byte[] byteArray = System.Text.Encoding.Default.GetBytes(strSendData);
                    uint success = _proxy.ICE_IPCSDK_TransSerialPort(pUid[i], byteArray, (UInt32)byteArray.Length);//发送rs485数据
                    //mutexThread.ReleaseMutex();
                    if (success == 1)
                    {
                        nGateNum[i]++;
        ...
        private void sendRS232()
        {
            while (true)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    byte[] byteArray = System.Text.Encoding.Default.GetBytes(strSendData);
                    uint success = _proxy.ICE_IPCSDK_TransSerialPort_RS232(pUid[i], byteArray, (UInt32)byteArray.Length);//发送rs232数据
                    //mutexThread.ReleaseMutex();
                    if (success == 1)
                    {
                        nGateNum[i]++;
        ...
        private void settingToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (form2.ShowDialog() == DialogResult.OK)
            {
                ...
                _proxy.ICE_IPCSDK_LogConfig(form2.m_bEnableLog, form2.m_strLogPath);
            }
            form2.Dispose();
        }
        //清零
        ...
        private void buttonRecord1_Click(object sender, EventArgs e)
        {
            ...
            if (!bRecord[0])//开始录像
            {
                try
                {
                    ...
                    uint flag = _proxy.ICE_IPCSDK_StartRecord(pUid[0], strVideoName);//开始录像
                    if (flag == 0)
                        return;
                    bRecord[0] = true;
            ...
            else//结束录像
            {
                ...
                {
                    bRecord[0] = false;
                    _proxy.ICE_IPCSDK_StopRecord(pUid[0]);
        ...
        private void buttonRecord2_Click(object sender, EventArgs e)
        {
            ...
            if (!bRecord[1])//开始录像
            {
                ...
                uint flag = _proxy.ICE_IPCSDK_StartRecord(pUid[1], strVideoName);//开始录像
            ...
            else//结束录像
            {
                ...
                _proxy.ICE_IPCSDK_StopRecord(pUid[1]);
            }
        }
        ...
        private void record1()
        {
            while (true)
            {
                ...
                if (strpath == "")
                    return;
                _proxy.ICE_IPCSDK_StartRecord(pUid[0], strpath);
            }
        }
        ...
        private void record2()
        {
            while (true)
            {
                ...
                _proxy.ICE_IPCSDK_StartRecord(pUid[1], strpath);
            }
        }
        ...
        private void record3()
        {
            while (true)
            {
                ...
                _proxy.ICE_IPCSDK_StopRecord(pUid[2]);
                string strpath = getVideoName(2);
                _proxy.ICE_IPCSDK_StartRecord(pUid[2], strpath);
            }
        }
        ...
        private void record4()
        {
            while (true)
            {
                ...
                _proxy.ICE_IPCSDK_StopRecord(pUid[3]);
                string strpath = getVideoName(3);
                _proxy.ICE_IPCSDK_StartRecord(pUid[3], strpath);
            }
        }
        ...
        private void buttonRecord3_Click(object sender, EventArgs e)
        {
            ...
            if (!bRecord[2])//开始录像
            {
                ...
                uint flag = _proxy.ICE_IPCSDK_StartRecord(pUid[2], strVideoName);
                if (flag == 0)
                    return;
        ...
        private void buttonRecord4_Click(object sender, EventArgs e)
        {
            ...
            if (!bRecord[3])//开始录像
            {
                ...
                uint flag = _proxy.ICE_IPCSDK_StartRecord(pUid[3], strVideoName);
                if (flag == 0)
                    return;
        ...
        private void buttonStartVideo1_Click(object sender, EventArgs e)
        {
            ...
            if (0 == _proxy.ICE_IPCSDK_GetStatus(pUid[0]))//获取相机连接状态
                return;
            ...
            if (!bPreview[0])//开始视频
            {
                ...
                if (videoHwnd != IntPtr.Zero)
                {
                    UInt32 ret = _proxy.ICE_IPCSDK_StartStream(pUid[0], 1, (UInt32)videoHwnd);//开始视频
                }
                buttonRecord1.Enabled = true;
                buttonStartVideo1.Text = "结束视频";
                bPreview[0] = true;
            ...
            else//关闭视频
            {
                if (bRecord[0])
                {
                    ...
                    _proxy.ICE_IPCSDK_StopRecord(pUid[0]);//没有开启视频的情况下不能录像
                }
                _proxy.ICE_IPCSDK_StopStream(pUid[0]);//关闭视频
                buttonStartVideo1.Text = "开始视频";
                bPreview[0] = false;
                buttonRecord1.Enabled = false;
            }
        ...
        private void buttonStartVideo2_Click(object sender, EventArgs e)
        {
            ...
            if (0 == _proxy.ICE_IPCSDK_GetStatus(pUid[1]))
                return;
            ...
            else//关闭视频
            {
                if (bRecord[1])
                {
                    ...
                    _proxy.ICE_IPCSDK_StopRecord(pUid[1]);//没有开启视频的情况下不能录像
                }
                _proxy.ICE_IPCSDK_StopStream(pUid[1]);
                buttonStartVideo2.Text = "开始视频";
                bPreview[1] = false;
                buttonRecord2.Enabled = false;
            }
        ...
        private void buttonStartVideo3_Click(object sender, EventArgs e)
        {
            ...
            if (0 == _proxy.ICE_IPCSDK_GetStatus(pUid[2]))
                return;
            ...
            else//关闭视频
            {
                if (bRecord[2])
                {
                    ...
                    _proxy.ICE_IPCSDK_StopRecord(pUid[2]);//没有开启视频的情况下不能录像
                }
                _proxy.ICE_IPCSDK_StopStream(pUid[2]);
                buttonStartVideo3.Text = "开始视频";
                bPreview[2] = false;
                buttonRecord3.Enabled = false;
            }
        ...
        private void buttonStartVideo4_Click(object sender, EventArgs e)
        {
            ...
            if (0 == _proxy.ICE_IPCSDK_GetStatus(pUid[3]))
                return;
            ...
            else//关闭视频
            {
                if (bRecord[3])
                {
                    ...
                    _proxy.ICE_IPCSDK_StopRecord(pUid[3]);//没有开启视频的情况下不能录像
                }
                _proxy.ICE_IPCSDK_StopStream(pUid[3]);
                buttonStartVideo4.Text = "开始视频";
                bPreview[3] = false;
                buttonRecord4.Enabled = false;
            }
        ...
        private void button_WR1_Click(object sender, EventArgs e)
        {
            ...
            _proxy.ICE_IPCSDK_WriteUserData(pUid[0], data);//写入数据

            byte[] recvData = new byte[256];
            _proxy.ICE_IPCSDK_ReadUserData(pUid[0], recvData, 256);//读取数据
            if (recvData == null)
            {
        ...
        private void button_WR2_Click(object sender, EventArgs e)
        {
            ...
            _proxy.ICE_IPCSDK_WriteUserData(pUid[1], data);//写入数据

            byte[] recvData = new byte[256];
            _proxy.ICE_IPCSDK_ReadUserData(pUid[1], recvData, 256);//读取数据
            if (recvData == null)
            {
                string errorData = "收取用户数据失败";
                listBoxInfo.Items.Insert(0, errorData);
        ...
        private void button_WR3_Click(object sender, EventArgs e)
        {
            ...
            _proxy.ICE_IPCSDK_WriteUserData(pUid[2], data);//写入数据

            byte[] recvData = new byte[256];
            _proxy.ICE_IPCSDK_ReadUserData(pUid[2], recvData, 256);//读取数据
            if (recvData == null)
            {
                string errorData = "收取用户数据失败";
                listBoxInfo.Items.Insert(0, errorData);
        ...
        private void button_WR4_Click(object sender, EventArgs e)
        {
            ...
            _proxy.ICE_IPCSDK_WriteUserData(pUid[3], data);//写入数据

            byte[] recvData = new byte[256];
            _proxy.ICE_IPCSDK_ReadUserData(pUid[3], recvData, 256);//读取数据
            if (recvData == null)
            {
                string errorData = "收取用户数据失败";
                listBoxInfo.Items.Insert(0, errorData);
        ...
        private void button_network1_Click(object sender, EventArgs e)
        {
            ...
            uint success = _proxy.ICE_IPCSDK_GetIPAddr(pUid[0], ref ip, ref mask, ref gateway);//获取相机网络参数
            if (success == 0)
            {
            ...
            //获取开关量输出配置
            success = _proxy.ICE_IPCSDK_GetAlarmOutConfig(pUid[0], 1, ref pu32IdleState, ref pu32DelayTime, ref reserve);
            if (success == 0)
            {
            ...
            success = _proxy.ICE_IPCSDK_GetTriggerMode(pUid[0], ref u32TriggerMode);
            if (success == 0)
            {
            ...
            success = _proxy.ICE_IPCSDK_GetUARTCfg(pUid[0], ref uart_param);
            if (0 == success)
            {
            ...
            uint nBottom = 0;
            uint nTop = 0;
            success = _proxy.ICE_IPCSDK_GetLoop(pUid[0], ref nLeft, ref nBottom, ref nRight, ref nTop, form.nWidth, form.nHeight);
            if (0 == success)
            {
            ...
            success = _proxy.ICE_IPCSDK_GetVehicleBrand(pUid[0], ref nFilterByPlate, ref nEnableNoPlateBrand, ref nEnableBrand);
            if (0 == success)
            {
            ...
            {
                //设置开关量输出参数
                success = _proxy.ICE_IPCSDK_SetAlarmOutConfig(pUid[0], 1, form.pu32IdleState, form.pu32DelayTime, reserve);
                if (success == 0)
            ...
            {
                //修改触发模式
                success = _proxy.ICE_IPCSDK_SetTriggerMode(pUid[0], form.u32TriggerMode);
                if (success == 0)
            ...
            if (form.nUartChange)
            {
                success = _proxy.ICE_IPCSDK_SetUARTCfg(pUid[0], ref form.uart_param);
                if (0 == success)
            ...
            if (form.bLoopChange)
            {
                success = _proxy.ICE_IPCSDK_SetLoop(pUid[0], form.nLeftx, form.nLefty, form.nRightx, form.nRighty, form.nWidth, form.nHeight);
                if (0 == success)
                    MessageBox.Show("设置串口参数失败");
            }

            //修改车款识别
            success = _proxy.ICE_IPCSDK_SetVehicleBrand(pUid[0], form.nFilterByPlate, form.nEnableNoPlateBrand, form.nEnableBrand);
            if (0 == success)
            ...
            {
                success = _proxy.ICE_IPCSDK_SetIPAddr(pUid[0], form.uIp, form.uMask, form.uGateway);//设置网络参数
                if (success == 0)
        ...
        private void button_network2_Click(object sender, EventArgs e)
        {
            ...
            uint success = _proxy.ICE_IPCSDK_GetIPAddr(pUid[1], ref ip, ref mask, ref gateway);//获取相机网络参数
            if (success == 0)
            {
                MessageBox.Show("获取ip失败！");
                return;
            ...
            //获取开关量输出配置
            success = _proxy.ICE_IPCSDK_GetAlarmOutConfig(pUid[1], 1, ref pu32IdleState, ref pu32DelayTime, ref reserve);
            if (success == 0)
            {
                MessageBox.Show("获取开关量输出配置失败！");
                return;
            ...
            uint u32TriggerMode = 0;
            success = _proxy.ICE_IPCSDK_GetTriggerMode(pUid[1], ref u32TriggerMode);
            if (success == 0)
            {
                MessageBox.Show("获取触发模式失败！");
                return;
            ...
            //串口设置
            ICE_UART_PARAM uart_param = new ICE_UART_PARAM();
            success = _proxy.ICE_IPCSDK_GetUARTCfg(pUid[1], ref uart_param);
            if (0 == success)
            {
                MessageBox.Show("获取串口参数失败");
                return;
            ...
            uint nBottom = 0;
            uint nTop = 0;
            success = _proxy.ICE_IPCSDK_GetLoop(pUid[1], ref nLeft, ref nBottom, ref nRight, ref nTop, form.nWidth, form.nHeight);
            if (0 == success)
            {
                MessageBox.Show("获取识别区域失败");
                return;
            ...
            //车款识别
            int nFilterByPlate = 0, nEnableNoPlateBrand = 0, nEnableBrand = 0;
            success = _proxy.ICE_IPCSDK_GetVehicleBrand(pUid[1], ref nFilterByPlate, ref nEnableNoPlateBrand, ref nEnableBrand);
            if (0 == success)
            {
                MessageBox.Show("获取配置车款识别参数失败");
                return;
            ...
            {
                //设置开关量输出参数
                success = _proxy.ICE_IPCSDK_SetAlarmOutConfig(pUid[1], 1, form.pu32IdleState, form.pu32DelayTime, reserve);
                if (success == 0)
                    MessageBox.Show("修改开关量失败！");
            }
            ...
            {
                //修改触发模式
                success = _proxy.ICE_IPCSDK_SetTriggerMode(pUid[1], form.u32TriggerMode);
                if (success == 0)
                    MessageBox.Show("修改触发模式失败！");
            }
            ...
            if (form.nUartChange)
            {
                success = _proxy.ICE_IPCSDK_SetUARTCfg(pUid[1], ref form.uart_param);
                if (0 == success)
                    MessageBox.Show("设置串口参数失败");
            }
            ...
            if (form.bLoopChange)
            {
                success = _proxy.ICE_IPCSDK_SetLoop(pUid[1], form.nLeftx, form.nLefty, form.nRightx, form.nRighty, form.nWidth, form.nHeight);
                if (0 == success)
                    MessageBox.Show("设置串口参数失败");
            }

            //修改车款识别
            success = _proxy.ICE_IPCSDK_SetVehicleBrand(pUid[1], form.nFilterByPlate, form.nEnableNoPlateBrand, form.nEnableBrand);
            if (0 == success)
                MessageBox.Show("设置配置车款识别参数失败");
            ...
            if ((ip != form.uIp) || (mask != form.uMask) || (gateway != form.uGateway))
            {
                success = _proxy.ICE_IPCSDK_SetIPAddr(pUid[1], form.uIp, form.uMask, form.uGateway);//设置网络参数
                if (success == 0)
                    MessageBox.Show("修改IP失败！");
            }
        }
        ...
        private void button_network3_Click(object sender, EventArgs e)
        {
            ...
            uint success = _proxy.ICE_IPCSDK_GetIPAddr(pUid[2], ref ip, ref mask, ref gateway);//获取相机网络参数
            if (success == 0)
            {
                MessageBox.Show("获取ip失败！");
                return;
            ...
            //获取开关量输出配置
            success = _proxy.ICE_IPCSDK_GetAlarmOutConfig(pUid[2], 1, ref pu32IdleState, ref pu32DelayTime, ref reserve);
            if (success == 0)
            {
                MessageBox.Show("获取开关量输出配置失败！");
                return;
            ...
            uint u32TriggerMode = 0;
            success = _proxy.ICE_IPCSDK_GetTriggerMode(pUid[2], ref u32TriggerMode);
            if (success == 0)
            {
                MessageBox.Show("获取触发模式失败！");
                return;
            ...
            //串口设置
            ICE_UART_PARAM uart_param = new ICE_UART_PARAM();
            success = _proxy.ICE_IPCSDK_GetUARTCfg(pUid[2], ref uart_param);
            if (0 == success)
            {
                MessageBox.Show("获取串口参数失败");
                return;
            ...
            uint nBottom = 0;
            uint nTop = 0;
            success = _proxy.ICE_IPCSDK_GetLoop(pUid[2], ref nLeft, ref nBottom, ref nRight, ref nTop, form.nWidth, form.nHeight);
            if (0 == success)
            {
                MessageBox.Show("获取识别区域失败");
                return;
            ...
            //车款识别
            int nFilterByPlate = 0, nEnableNoPlateBrand = 0, nEnableBrand = 0;
            success = _proxy.ICE_IPCSDK_GetVehicleBrand(pUid[2], ref nFilterByPlate, ref nEnableNoPlateBrand, ref nEnableBrand);
            if (0 == success)
            {
                MessageBox.Show("获取配置车款识别参数失败");
                return;
            ...
            {
                //设置开关量输出参数
                success = _proxy.ICE_IPCSDK_SetAlarmOutConfig(pUid[2], 1, form.pu32IdleState, form.pu32DelayTime, reserve);
                if (success == 0)
                    MessageBox.Show("修改开关量失败！");
            }
            ...
            {
                //修改触发模式
                success = _proxy.ICE_IPCSDK_SetTriggerMode(pUid[2], form.u32TriggerMode);
                if (success == 0)
                    MessageBox.Show("修改触发模式失败！");
            }
            ...
            if (form.nUartChange)
            {
                success = _proxy.ICE_IPCSDK_SetUARTCfg(pUid[2], ref form.uart_param);
                if (0 == success)
                    MessageBox.Show("设置串口参数失败");
            }
            ...
            if (form.bLoopChange)
            {
                success = _proxy.ICE_IPCSDK_SetLoop(pUid[2], form.nLeftx, form.nLefty, form.nRightx, form.nRighty, form.nWidth, form.nHeight);
                if (0 == success)
                    MessageBox.Show("设置串口参数失败");
            }

            //修改车款识别
            success = _proxy.ICE_IPCSDK_SetVehicleBrand(pUid[2], form.nFilterByPlate, form.nEnableNoPlateBrand, form.nEnableBrand);
            if (0 == success)
                MessageBox.Show("设置配置车款识别参数失败");
            ...
            if ((ip != form.uIp) || (mask != form.uMask) || (gateway != form.uGateway))
            {
                success = _proxy.ICE_IPCSDK_SetIPAddr(pUid[2], form.uIp, form.uMask, form.uGateway);//设置网络参数
                if (success == 0)
                    MessageBox.Show("修改IP失败！");
            }
        }
        ...
        private void button_network4_Click(object sender, EventArgs e)
        {
            ...
            uint success = _proxy.ICE_IPCSDK_GetIPAddr(pUid[3], ref ip, ref mask, ref gateway);//获取相机网络参数
            if (success == 0)
            {
                MessageBox.Show("获取ip失败！");
                return;
            ...
            //获取开关量输出配置
            success = _proxy.ICE_IPCSDK_GetAlarmOutConfig(pUid[3], 1, ref pu32IdleState, ref pu32DelayTime, ref reserve);
            if (success == 0)
            {
                MessageBox.Show("获取开关量输出配置失败！");
                return;
            ...
            uint u32TriggerMode = 0;
            success = _proxy.ICE_IPCSDK_GetTriggerMode(pUid[3], ref u32TriggerMode);
            if (success == 0)
            {
                MessageBox.Show("获取触发模式失败！");
                return;
            ...
            //串口设置
            ICE_UART_PARAM uart_param = new ICE_UART_PARAM();
            success = _proxy.ICE_IPCSDK_GetUARTCfg(pUid[3], ref uart_param);
            if (0 == success)
            {
                MessageBox.Show("获取串口参数失败");
                return;
            ...
            uint nBottom = 0;
            uint nTop = 0;
            success = _proxy.ICE_IPCSDK_GetLoop(pUid[3], ref nLeft, ref nBottom, ref nRight, ref nTop, form.nWidth, form.nHeight);
            if (0 == success)
            {
                MessageBox.Show("获取识别区域失败");
                return;
            ...
            //车款识别
            int nFilterByPlate = 0, nEnableNoPlateBrand = 0, nEnableBrand = 0;
            success = _proxy.ICE_IPCSDK_GetVehicleBrand(pUid[3], ref nFilterByPlate, ref nEnableNoPlateBrand, ref nEnableBrand);
            if (0 == success)
            {
                MessageBox.Show("获取配置车款识别参数失败");
                return;
            ...
            {
                //设置开关量输出参数
                success = _proxy.ICE_IPCSDK_SetAlarmOutConfig(pUid[3], 1, form.pu32IdleState, form.pu32DelayTime, reserve);
                if (success == 0)
                    MessageBox.Show("修改开关量失败！");
            }
            ...
            {
                //修改触发模式
                success = _proxy.ICE_IPCSDK_SetTriggerMode(pUid[3], form.u32TriggerMode);
                if (success == 0)
                    MessageBox.Show("修改触发模式失败！");
            }
            ...
            if (form.nUartChange)
            {
                success = _proxy.ICE_IPCSDK_SetUARTCfg(pUid[3], ref form.uart_param);
                if (0 == success)
                    MessageBox.Show("设置串口参数失败");
            }
            ...
            if (form.bLoopChange)
            {
                success = _proxy.ICE_IPCSDK_SetLoop(pUid[3], form.nLeftx, form.nLefty, form.nRightx, form.nRighty, form.nWidth, form.nHeight);
                if (0 == success)
                    MessageBox.Show("设置串口参数失败");
            }

            //修改车款识别
            success = _proxy.ICE_IPCSDK_SetVehicleBrand(pUid[3], form.nFilterByPlate, form.nEnableNoPlateBrand, form.nEnableBrand);
            if (0 == success)
                MessageBox.Show("设置配置车款识别参数失败");
            ...
            if ((ip != form.uIp) || (mask != form.uMask) || (gateway != form.uGateway))
            {
                success = _proxy.ICE_IPCSDK_SetIPAddr(pUid[3], form.uIp, form.uMask, form.uGateway);//设置网络参数
                if (success == 0)
                    MessageBox.Show("修改IP失败！");
            }
        }
        ...
        private void buttonOpenGate2_1_Click(object sender, EventArgs e)
        {
            if (pUid[0] != IntPtr.Zero)
            {
                _proxy.ICE_IPCSDK_ControlAlarmOut(pUid[0], 1);//控制IO2输出
            }
        }
        ...
        private void button_openGate2_2_Click(object sender, EventArgs e)
        {
            if (pUid[1] != IntPtr.Zero)
            {
                _proxy.ICE_IPCSDK_ControlAlarmOut(pUid[1], 1);//控制IO2输出
            }
        }
        ...
        private void button_openGate2_3_Click(object sender, EventArgs e)
        {
            if (pUid[2] != IntPtr.Zero)
            {
                _proxy.ICE_IPCSDK_ControlAlarmOut(pUid[2], 1);//控制IO2输出
            }
        }
        ...
        private void button_openGate2_4_Click(object sender, EventArgs e)
        {
            if (pUid[3] != IntPtr.Zero)
            {
                _proxy.ICE_IPCSDK_ControlAlarmOut(pUid[3], 1);//控制IO2输出
            }
        }
        ...
        private void button_broadcast1_Click(object sender, EventArgs e)
        {
            if (pUid[0] == IntPtr.Zero)
                return;
            _proxy.ICE_IPCSDK_Broadcast(pUid[0], (ushort)comboBox_BIndex1.SelectedIndex);//语音播报
        }
        ...
        private void button_broadcast2_Click(object sender, EventArgs e)
        {
            if (pUid[1] == IntPtr.Zero)
                return;
            _proxy.ICE_IPCSDK_Broadcast(pUid[1], (ushort)comboBox_BIndex2.SelectedIndex);//语音播报
        }
        ...
        private void button_broadcast3_Click(object sender, EventArgs e)
        {
            if (pUid[2] == IntPtr.Zero)
                return;
            _proxy.ICE_IPCSDK_Broadcast(pUid[2], (ushort)comboBox_BIndex3.SelectedIndex);//语音播报
        }
        ...
        private void button_broadcast4_Click(object sender, EventArgs e)
        {
            if (pUid[3] == IntPtr.Zero)
                return;
            _proxy.ICE_IPCSDK_Broadcast(pUid[3], (ushort)comboBox_BIndex4.SelectedIndex);//语音播报
        }
        ...
        private void button_RS232_1_Click(object sender, EventArgs e)
        {
            ...
            byte[] byteArray = System.Text.Encoding.Default.GetBytes(strSendData);
            //发送RS232数据
            _proxy.ICE_IPCSDK_TransSerialPort_RS232(pUid[0], byteArray, (UInt32)byteArray.Length);
            ...
            _proxy.ICE_IPCSDK_TransSerialPort_RS232(pUid[0], byteArray_16, (UInt32)byteArray_16.Length);
        }
        ...
        private void button_RS232_2_Click(object sender, EventArgs e)
        {
            ...
            byte[] byteArray = System.Text.Encoding.Default.GetBytes(strSendData);
            //发送RS232数据
            _proxy.ICE_IPCSDK_TransSerialPort_RS232(pUid[1], byteArray, (UInt32)byteArray.Length);
            ...
            _proxy.ICE_IPCSDK_TransSerialPort_RS232(pUid[1], byteArray_16, (UInt32)byteArray_16.Length);
        }
        ...
        private void button_RS232_3_Click(object sender, EventArgs e)
        {
            ...
            byte[] byteArray = System.Text.Encoding.Default.GetBytes(strSendData);
            //发送RS232数据
            _proxy.ICE_IPCSDK_TransSerialPort_RS232(pUid[2], byteArray, (UInt32)byteArray.Length);
            ...
            _proxy.ICE_IPCSDK_TransSerialPort_RS232(pUid[2], byteArray_16, (UInt32)byteArray_16.Length);
        }
        ...
        private void button_RS232_4_Click(object sender, EventArgs e)
        {
            ...
            byte[] byteArray = System.Text.Encoding.Default.GetBytes(strSendData);
            //发送RS232数据
            _proxy.ICE_IPCSDK_TransSerialPort_RS232(pUid[3], byteArray, (UInt32)byteArray.Length);
            ...
            _proxy.ICE_IPCSDK_TransSerialPort_RS232(pUid[3], byteArray_16, (UInt32)byteArray_16.Length);
        }
        ...
        private void button_WBinary1_Click(object sender, EventArgs e)
        {
            ...
            recvData.Initialize();
            _proxy.ICE_IPCSDK_ReadUserData(pUid[0], recvData, 4096);//读取用户数据
            ...
            flag = _proxy.ICE_IPCSDK_WriteUserData_Binary(pUid[0], data, (uint)nOffset, (uint)len);//写入二进制数据
            if (flag == 0)
            {
            ...
            flag = _proxy.ICE_IPCSDK_ReadUserData(pUid[0], recvData, 4096);//读取用户数据
            if (flag == 0)
        ...
        private void button_WBinary2_Click(object sender, EventArgs e)
        {
            ...
            byte[] recvData = new byte[4096];
            recvData.Initialize();
            _proxy.ICE_IPCSDK_ReadUserData(pUid[1], recvData, 4096);
            ...
            len = System.Text.Encoding.Default.GetByteCount(data);
            flag = _proxy.ICE_IPCSDK_WriteUserData_Binary(pUid[1], data, (uint)nOffset, (uint)len);//写入二进制数据
            if (flag == 0)
            {
                listBoxInfo.Items.Insert(0, "写入二进制数据失败");
                return;
            ...
            Array.Clear(recvData, 0, recvData.Length);
            flag = _proxy.ICE_IPCSDK_ReadUserData(pUid[1], recvData, 4096);//读取用户数据
            if (flag == 0)
                listBoxInfo.Items.Insert(0, "读取数据失败");
            else
            {
        ...
        private void button_WBinary3_Click(object sender, EventArgs e)
        {
            ...
            byte[] recvData = new byte[4096];
            recvData.Initialize();
            _proxy.ICE_IPCSDK_ReadUserData(pUid[2], recvData, 4096);
            ...
            len = System.Text.Encoding.Default.GetByteCount(data);
            flag = _proxy.ICE_IPCSDK_WriteUserData_Binary(pUid[2], data, (uint)nOffset, (uint)len);//写入二进制数据
            if (flag == 0)
            {
                listBoxInfo.Items.Insert(0, "写入二进制数据失败");
                return;
            ...
            Array.Clear(recvData, 0, recvData.Length);
            flag = _proxy.ICE_IPCSDK_ReadUserData(pUid[2], recvData, 4096);//读取用户数据
            if (flag == 0)
                listBoxInfo.Items.Insert(0, "读取数据失败");
            else
            {
        ...
        private void button_WBinary4_Click(object sender, EventArgs e)
        {
            ...
            byte[] recvData = new byte[4096];
            recvData.Initialize();
            _proxy.ICE_IPCSDK_ReadUserData(pUid[3], recvData, 4096);
            ...
            len = System.Text.Encoding.Default.GetByteCount(data);
            flag = _proxy.ICE_IPCSDK_WriteUserData_Binary(pUid[3], data, (uint)nOffset, (uint)len);//写入二进制数据
            if (flag == 0)
            {
                listBoxInfo.Items.Insert(0, "写入二进制数据失败");
                return;
            ...
            Array.Clear(recvData, 0, recvData.Length);
            flag = _proxy.ICE_IPCSDK_ReadUserData(pUid[3], recvData, 4096);//读取用户数据
            if (flag == 0)
                listBoxInfo.Items.Insert(0, "读取数据失败");
            else
            {
        ...
        private void button_RBinary1_Click(object sender, EventArgs e)
        {
            ...
            Array.Clear(recvData, 0, recvData.Length);
            uint success = _proxy.ICE_IPCSDK_ReadUserData_Binary(pUid[0], recvData, 4096, (uint)nOffset, (uint)recvLen);//读取二进制数据
            if (success == 0)
            {
        ...
        private void button_RBinary2_Click(object sender, EventArgs e)
        {
            ...
            Array.Clear(recvData, 0, recvData.Length);
            uint success = _proxy.ICE_IPCSDK_ReadUserData_Binary(pUid[1], recvData, 4096, (uint)nOffset, (uint)recvLen);//读取二进制数据
            if (success == 0)
            {
                string errorData = "收取二进制数据失败";
                listBoxInfo.Items.Insert(0, errorData);
        ...
        private void button_RBinary3_Click(object sender, EventArgs e)
        {
            ...
            Array.Clear(recvData, 0, recvData.Length);
            uint success = _proxy.ICE_IPCSDK_ReadUserData_Binary(pUid[2], recvData, 4096, (uint)nOffset, (uint)recvLen);//读取二进制数据
            if (success == 0)
            {
                string errorData = "收取二进制数据失败";
                listBoxInfo.Items.Insert(0, errorData);
        ...
        private void button_RBinary4_Click(object sender, EventArgs e)
        {
            ...
            Array.Clear(recvData, 0, recvData.Length);
            uint success = _proxy.ICE_IPCSDK_ReadUserData_Binary(pUid[3], recvData, 4096, (uint)nOffset, (uint)recvLen);//读取二进制数据
            if (success == 0)
            {
                string errorData = "收取二进制数据失败";
                listBoxInfo.Items.Insert(0, errorData);
        ...
        {
            int index = (int)msg;
            _proxy.ICE_IPCSDK_BroadcastGroup(pUid[index], textBox_groupIndex.Text);//语言组播
        ...
        private void button_cameraInfo1_Click(object sender, EventArgs e)
        {
            ...
            uint success = _proxy.ICE_IPCSDK_GetCameraInfo(pUid[0], ref info);
            if (success == 1)
            {
        ...
        {
            ICE_CameraInfo info = new ICE_CameraInfo();
            uint success = _proxy.ICE_IPCSDK_GetCameraInfo(pUid[1], ref info);
            if (success == 1)
            {
                listBoxInfo.Items.Insert(0, "版本时间：" + info.szAppTime);
                if (0 == info.szIsEnc)
        ...
        {
            ICE_CameraInfo info = new ICE_CameraInfo();
            uint success = _proxy.ICE_IPCSDK_GetCameraInfo(pUid[2], ref info);
            if (success == 1)
            {
                listBoxInfo.Items.Insert(0, "版本时间：" + info.szAppTime);
                if (0 == info.szIsEnc)
        ...
        {
            ICE_CameraInfo info = new ICE_CameraInfo();
            uint success = _proxy.ICE_IPCSDK_GetCameraInfo(pUid[3], ref info);
            if (success == 1)
            {
                listBoxInfo.Items.Insert(0, "版本时间：" + info.szAppTime);
                if (0 == info.szIsEnc)
        ...
        private void button_getIOState1_Click(object sender, EventArgs e)
        {
            ...
            for (i = 0; i < 4; i++)
            {
                success = _proxy.ICE_IPCSDK_GetIOState(pUid[0], i, ref u32IOState, ref reserve1, ref reserve2);
                if (1 == success)
        ...
        private void button_getIOState2_Click(object sender, EventArgs e)
        {
            ...
            for (i = 0; i < 4; i++)
            {
                success = _proxy.ICE_IPCSDK_GetIOState(pUid[1], i, ref u32IOState, ref reserve1, ref reserve2);
                if (1 == success)
                    strText += "IO" + (i + 1).ToString() + "状态：" + u32IOState.ToString() + " ";
                else
                    strText += "IO" + (i + 1).ToString() + "状态获取失败 ";
        ...
        private void button_getIOState3_Click(object sender, EventArgs e)
        {
            ...
            for (i = 0; i < 4; i++)
            {
                success = _proxy.ICE_IPCSDK_GetIOState(pUid[2], i, ref u32IOState, ref reserve1, ref reserve2);
                if (1 == success)
                    strText += "IO" + (i + 1).ToString() + "状态：" + u32IOState.ToString() + " ";
                else
                    strText += "IO" + (i + 1).ToString() + "状态获取失败 ";
        ...
        private void button_getIOState4_Click(object sender, EventArgs e)
        {
            ...
            for (i = 0; i < 4; i++)
            {
                success = _proxy.ICE_IPCSDK_GetIOState(pUid[3], i, ref u32IOState, ref reserve1, ref reserve2);
                if (1 == success)
                    strText += "IO" + (i + 1).ToString() + "状态：" + u32IOState.ToString() + " ";
                else
                    strText += "IO" + (i + 1).ToString() + "状态获取失败 ";
        ...
        private void getVehicleInfo(int index)
        {
            ...
            uint success = _proxy.ICE_IPCSDK_getOfflineVehicleInfo(pUid[index], buf, 8 * 1024 * 1024, ref len);
            if (success <= 0)
            {
        ...
        void getPayInfo(int index)
        {
            ...
            uint success = _proxy.ICE_IPCSDK_getPayInfo(pUid[index], buf, 8 * 1024 * 1024, ref len);
            if (success <= 0)
            {
        ...
        private void button_updateList_Click(object sender, EventArgs e)
        {
            ...
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                ...
                uint ret = _proxy.ICE_IPCSDK_UpdateWhiteListBatch(pUid[0], strFileName, 1);
